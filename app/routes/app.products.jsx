...
// Import Shopify Polaris Components (Added Modal)
import { 
...
export async function loader({ request }) {
  ...
  console.log("Admin authenticated, session:", session);
...
export async function action({ request }) {
  ...
  if (type === "full_sync") {
    ...
    for (const p of response.data.products) {
      await db.product.upsert({
        ...
        update: { title: p.title, description: p.body_html, vendor: p.vendor, product_type: p.product_type, status: p.status, price: parseFloat(p.variants[0]?.price || 0), sku: p.variants[0]?.sku || "", updated_at: new Date(p.updated_at) },
        create: { shopify_product_id: p.id, title: p.title, description: p.body_html, vendor: p.vendor, product_type: p.product_type, status: p.status, price: parseFloat(p.variants[0]?.price || 0), sku: p.variants[0]?.sku || "", created_at: new Date(p.created_at), updated_at: new Date(p.updated_at) },
      });
    }
    return { success: true };
  }
  ...
  if (type === "create") {
    ...
    await db.product.create({
      data: { shopify_product_id: p.id, title: p.title, description: p.body_html, vendor: p.vendor, product_type: p.product_type, status: p.status, price: parseFloat(p.variants[0].price), sku: p.variants[0].sku, created_at: new Date(p.created_at), updated_at: new Date(p.updated_at) },
    });
    return { success: true };
  }
  ...
  if (type === "update") {
    ...
    await db.product.updateMany({
      ...
      data: { title: formData.get("title"), description: formData.get("description"), vendor: formData.get("vendor"), product_type: formData.get("product_type"), status: formData.get("status"), price: parseFloat(formData.get("price")), sku: formData.get("sku"), updated_at: new Date() },
    });
    return { success: true };
  }
  ...
  if (type === "delete") {
    ...
    await db.product.deleteMany({ where: { shopify_product_id: shopifyId } });
    return { success: true };
  }
...
export default function ProductsPage() {
  ...
  const handleCreateChange = (value, id) => setCreateForm(prev => ({ ...prev, [id]: value }));
  ...
  const handleCreate = () => {
    ...
    setCreateForm({ title: "", description: "", vendor: "", product_type: "", sku: "", price: "", status: "active" }); // Reset form
    setIsModalOpen(false); // Close Modal after saving
  };

  const startEditing = (p) => {
    setEditingId(p.id);
    setEditForm({ ...p, price: p.price.toString() }); // Load product data into edit state
  };
  ...
  return (
    ...
    >
                ...
                return (
                  <IndexTable.Row id={p.id} key={p.id} position={p.id}>
                    {isEditing ? (
                      ...
                      <>
                        <IndexTable.Cell>
                          {/* BlockStack fixes the cramped text issue */}
                          <BlockStack gap="100">
      ...
      {/* 🚀 MODAL POPUP FOR ADDING PRODUCTS */}
      <Modal
        open={isModalOpen}
      ...
      >
        <Modal.Section>
          <FormLayout>
            <FormLayout.Group>
              <TextField label="Title" value={createForm.title} onChange={handleCreateChange} id="title" autoComplete="off" />
              <TextField label="Price (₹)" type="number" value={createForm.price} onChange={handleCreateChange} id="price" autoComplete="off" />
            </FormLayout.Group>
            <TextField label="Description" value={createForm.description} onChange={handleCreateChange} id="description" multiline={3} autoComplete="off" />
            <FormLayout.Group>
              <TextField label="Vendor" value={createForm.vendor} onChange={handleCreateChange} id="vendor" autoComplete="off" />
              <TextField label="Product Type" value={createForm.product_type} onChange={handleCreateChange} id="product_type" autoComplete="off" />
            </FormLayout.Group>
            <FormLayout.Group>
              <TextField label="SKU" value={createForm.sku} onChange={handleCreateChange} id="sku" autoComplete="off" />
              {/* <Select label="Status" options={[{label: 'Active', value: 'active'}, {label: 'Draft', value: 'draft'}]} value={createForm.status} onChange={handleCreateChange} id="status" /> */}
            </FormLayout.Group>
          </FormLayout>
        </Modal.Section>
      </Modal>
